...
Object-aware prompt, dense frames (Stage B)            & $0.8118$ & $0.8468$ & $-0.0350$ & $1/4/15$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.55\linewidth]{qualitative.png}
\caption{Qualitative samples (object-aware prompt). Each pair shows
the graph-conditioned generation (left) and the text-only baseline
(right) for the same prompt and seed; the caption underneath the pair
labels the corresponding test video and the recall delta. The same
seed is used so any difference is attributable to the graph stream.}
\label{fig:qual}
\end{figure}
...
\paragraph{Stage B: dense-frame retraining.}
The experiments above (Stage A) use only the $\sim 15$ AG-annotated
frames per video. We then extracted every sixth raw frame from each
Charades source video into a \texttt{frames\_full} directory and
modified \texttt{AGImageDataset} to sample any of those dense frames as
the target image, snapping \texttt{target\_frame\_idx} to the nearest
annotated frame so the temporal graph still indexes a valid node
bucket. This raises the effective $(\text{frame},\text{graph})$ pair
count by $\sim 6\!\times\!$ per video without any new annotation. Of
the $7\,427$ AG-train videos we successfully covered $5\,566$ at the
time of retraining (the remaining $\sim 25\%$ fall back to annotated
frames). With the same adapter, mix ratio, learning rate and
early-stopping policy as Stage A, validation MSE bottoms out at
$0.0866$ at step $1\,000$---compared to $0.0874$ at step $4\,000$ for
Stage A. The denser data therefore lets the adapter saturate
$\sim\!4\!\times\!$ faster, with a marginally lower minimum, and total
training wall-clock drops from $\sim\!3.5\,\text{h}$ to $2\,\text{h}\
22\,\text{min}$ on the same A100. Object Recall on the same $20$ test
graphs (Table~\ref{tab:recall}, row 4) is $0.8118$ for graph-cond vs.\
$0.8468$ for the bit-identical text-only baseline---essentially the
same saturated regime as row $3$, with a $1/4/15$ W/L/T split that is
again dominated by ties at recall $\geq 0.8$. We read this consistently
with the discussion of row $3$: dense data buys training-efficiency
gains, but the metric is too coarse in the object-aware regime to
expose them. We expect a relation-aware metric (future work item $3$
below) to be the right place to look.
...
A first roadmap item---a $\sim 6\!\times$ data scale-up by sampling
every sixth raw Charades frame as a target for the same temporal
graph---we executed and report as Stage B in
Section~\ref{sec:exploration}. The dense data shrinks training time
$\sim 4\!\times$ but does not move the saturated Object Recall, which
matches our claim that Object Recall is the wrong instrument for the
object-aware regime. Remaining extensions are: (1) Visual Genome
...
(2) an explicit relation-correctness metric---e.g., a vision-language
...
saturated regime; and (3) an ablation that rewinds the temporal edges
